...
(async () => {
    ...
    try {
        ...
        // 6. Create Students (some normal, some with concession, some Hifz enrolled)
        ...
        const students = [
            ...
            { name: 'Hamza Yusuf', class: 'Hifz Class', reg: 'REG-103', fee: null, concession: 0, notes: null },
            { name: 'Bilal Khan', class: 'Hifz Class', reg: 'REG-104', fee: 1000.00, concession: 1, notes: 'Sibling Discount' }
        ];
        const studentMap = {};
        for (const s of students) {
            ...
            // Enroll Hifz students in the Hifz engine tables
            if (s.class === 'Hifz Class') {
                await conn.execute(
                    `INSERT INTO hifz_enrollment (tenant_id, student_id, class_id, current_phase, current_para, total_lines_memorized, status, enrolled_date)
                     VALUES (?, ?, ?, 'tajweed_correction', 1, 0, 'active', '2025-01-15')`,
                    [tenantId, res.insertId, classId]
                );
            }
        }
        ...
        for (const m of months) {
            for (const s of students) {
                ...
                // Suppose Ali pays full fee (2000), Fatima pays concession (1200), Hifz Hamza (2500), Hifz Bilal (1000)
                let payAmt = s.fee;
                if (payAmt === null) {
                    payAmt = s.class === 'Class One' ? 2000.00 : 2500.00;
                }
                // Skip June payments randomly to simulate unpaid slots
                if (m === 6 && s.name === 'Muhammad Ali') continue;
